...
// because of the insane profit margins. You end up hitting the jackpot, breaking into the world's largest
...
function maxValue ( types, capacity ) {
  for ( var i = 0; i < types.length; i++ ) {
    if ( types[i].weight === 0 ) throw new Error("cake can't weigh nothing!");
    types[i].ratio = types[i].value / types[i].weight;
  }
  ...
  for ( var i = 0; i < types.length; i++ ) {
    if ( capacity !== 0 ) {
      num = Math.floor(capacity / types[i].weight);
      max += num * types[i].value;
      capacity = capacity - ( num * types[i].weight );
    }
  }
  return max;
}
...
function maxDuffelBagValue(cakeTypes, weightCapacity) {
  ...
  for (var i = 0; i <= weightCapacity; i++) { maxValuesAtCapacities[i] = 0; }
